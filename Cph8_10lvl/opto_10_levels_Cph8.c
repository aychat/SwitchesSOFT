...
int main(int argc, char *argv[])
{
	...
	double gamma_5_6  = 1./(freq_central*1.);		// 1 fs

	double gamma_9_6  = 1./(freq_central*10.);		// 10 fs
	double gamma_7_8  = 1./(freq_central*50.);     	 	// 50 fs
	double gamma_8_10 = 1./(freq_central*1.5*1000.);	// 1.5 ps
	double gamma_8_9  = 1./(freq_central*300.);		// 300 fs
	double gamma_10_1 = 1./(freq_central*1.);		// 1 fs
	...
	const double dt = 1.;
	const int Tsteps_zero_field = 150000-Tsteps;
	...
	double detuning = 0.005;
	gsl_vector *frequency = gsl_vector_alloc(Nfreq);

	gsl_vector_set(frequency, 0, E[1]-E[0]);
	gsl_vector_set(frequency, 1, E[1]-E[0]+detuning);
	gsl_vector_set(frequency, 2, E[1]-E[0]-detuning);

	gsl_vector_set(frequency, 3, E[2]-E[3]);
	gsl_vector_set(frequency, 4, E[2]-E[3]+detuning);
	gsl_vector_set(frequency, 5, E[2]-E[3]-detuning);

	gsl_vector_set(frequency, 6, E[6]-E[5]);
	gsl_vector_set(frequency, 7, E[6]-E[5]+detuning);
	gsl_vector_set(frequency, 8, E[6]-E[5]-detuning);

	gsl_vector_set(frequency, 9, E[7]-E[8]);
	gsl_vector_set(frequency, 10, E[7]-E[8]+detuning);
	gsl_vector_set(frequency, 11, E[7]-E[8]-detuning);

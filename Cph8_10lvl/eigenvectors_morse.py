...
if __name__ == '__main__':
    ...
    harmonic_osc = MUBQHamiltonian(
                        ...
                        omega=omega,
                        # V="0.5 * (omega * X) ** 2",
                        V="20. * omega * (1 - exp(-sqrt(omega/40)*(X)))**2",
                        K="0.5 * P ** 2",
                    )

    # plot eigenfunctions
    for n in range(10):
        plt.plot(harmonic_osc.X, harmonic_osc.get_eigenstate(n).real
                 + harmonic_osc.get_energy(n).real, label=str(n))
    plt.plot(harmonic_osc.X, 20. * harmonic_osc.omega * (1 - np.exp(-np.sqrt(harmonic_osc.omega/40)*(harmonic_osc.X)))**2)
    ...
    plt.ylim(0.0, 10*harmonic_osc.omega)
    plt.legend()
    plt.show()
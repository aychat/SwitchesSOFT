...
int main(int argc, char *argv[])
{
	...
	for (i=0; i<N; i++)
	{
	    ...
	    {
		fscanf(mu_matrix, "%lf ", &var);
		gsl_matrix_complex_set(mu_init, i, j, gsl_complex_rect(GSL_REAL(gsl_matrix_complex_get(mu_init, i, j)), var));
	    }
	}
	fclose(mu_matrix);

    printf("Dipole matrix \n");
	...
	{
		fscanf(A_matrix, "%lf ", &var);
		gsl_vector_complex_set(A_matrix_whole, i, gsl_complex_rect(GSL_REAL(gsl_vector_complex_get(A_matrix_whole, i)), var));
	}
	fclose(A_matrix);

...
if __name__ == '__main__':
    ...
    qsys_params = dict(
        ...
        codeVe="0.5*(self.freqVe*(q - self.displacement_ge))**2 + 2.",
        codeVge="-0.*q*self.field(t)",
        codefield=".0*np.exp(-0.00001*(t - 0.5*self.dt*self.Tsteps)**2)*(np.cos(.3*t) + np.cos((.3 + self.delt)*t))"
    )
    ...
    spectra = np.empty(molecule.Tsteps)
    ...
    for i in range(molecule.Tsteps):
        ...
        spectra[i] = (gibbs_state*molecule.rho_g.T).sum()

    plt.plot(molecule.Xrange, np.diag(molecule.rho_g).real, 'k-', label='Final ground state')
    plt.plot(molecule.Xrange, np.diag(molecule.rho_e).real, 'r', label='Final excited state')
    plt.legend()

    minus = (-1)**np.arange(molecule.Tsteps)

    freq = np.fft.fftshift(2.*np.pi*np.fft.fftfreq(molecule.Tsteps, molecule.dt))

    plt.figure()
    plt.subplot(211)
    plt.plot(freq, np.abs(minus*np.fft.fft(minus*molecule.Tsteps*spectra)))
    plt.subplot(212)
    plt.plot(spectra)
    plt.show()

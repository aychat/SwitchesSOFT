...
if __name__ == '__main__':
    ...
    qsys_params = dict(
        ...
        codefield="10.*np.exp(-(1./self.field_sigma2)*(t - 0.5*self.dt*self.Tsteps)**2)*np.cos((self.Ediff)*t)"
    )
    start = time.time()
    ...
    t = np.linspace(0.0, molecule.dt*molecule.Tsteps -molecule.dt, molecule.Tsteps)
    ...
    for j in range(molecule.Tsteps):
        ...
        spectra[j] = np.trace(molecule.rho_e).real

    d_omega = np.pi / (molecule.dt*molecule.Tsteps)

    omega_m = d_omega*molecule.Tsteps / 2.
    omega = np.linspace(-omega_m, omega_m - d_omega, molecule.Tsteps)

    exp_omega = np.exp(-0.5j*omega*molecule.Tsteps*molecule.dt)

    plt.figure()
    plt.plot(t, spectra)

    minus = (-1)**np.arange(molecule.Tsteps)
    A = np.fft.fft(spectra)
    plt.figure()
    plt.plot(omega, np.abs(minus*np.fft.fft(minus*spectra)))
